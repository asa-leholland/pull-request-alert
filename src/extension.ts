...
async function checkDiff() {
	...
	try {
		...
		const branchPoint = await executeCommand(`git merge-base ${currentBranch.trim()} develop`, currentWorkspace.uri.fsPath);
		...
		if (changes >= threshold) {
			vscode.window.showWarningMessage(`The sum of changes is ${changes}, it's time to submit a PR`);
		} else {
